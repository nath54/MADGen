"""
Runner script for Paradigm 1: Diarize-Then-Transcribe.
"""
...
from models.silero_whisper.pipeline import (
...
def main() -> None:
    """
    CLI entry point for Paradigm 1 evaluation.
    """

    parser = argparse.ArgumentParser(description="Evaluate Paradigm 1: Diarize-Then-Transcribe")
    parser.add_argument(
        "--sample-dir",
        type=str,
        default="data/output/sample_007",
        help="Path to sample directory containing mixed.wav, transcripts.jsonl, diarization.rttm",
    )
    parser.add_argument(
        "--whisper-size",
        type=str,
    ...
    pipeline = SileroWhisperPipeline(weights_manager=weights, whisper_model_size=args.whisper_size)
    ...
    if args.streaming:
        ...
        logger.info("Executing offline batch pipeline...")
    ...
    print("\n" + "=" * 70)
    print(f" PARADIGM 1 EVALUATION REPORT: {output.session_id}")
    print("=" * 70)
