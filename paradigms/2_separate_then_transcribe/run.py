"""
Runner script for Paradigm 2: Separate-Then-Transcribe.
"""
...
from models.oracle_irm_whisper.pipeline import OracleIRMWhisperPipeline
from models.sepformer_whisper.pipeline import SepformerWhisperPipeline
...
def main() -> None:
    """
    CLI entry point for Paradigm 2 evaluation.
    """

    parser = argparse.ArgumentParser(description="Evaluate Paradigm 2: Separate-Then-Transcribe")
    parser.add_argument(
        "--sample-dir",
        type=str,
        default="data/output/sample_001",
        help="Path to sample directory containing mixed_scene.wav, isolated_speakers/, transcripts.jsonl",
    )
    parser.add_argument(
        "--model",
        type=str,
        default="oracle_irm",
        choices=["oracle_irm", "sepformer"],
        help="Separation model to evaluate",
    )
    parser.add_argument(
        "--whisper-size",
        type=str,
    ...
    logger.info("Initializing Paradigm 2 Pipeline (Model: %s, Whisper size: %s)...", args.model, args.whisper_size)
    weights = WeightsManager()

    if args.model == "sepformer":
        pipeline = SepformerWhisperPipeline(weights_manager=weights, whisper_model_size=args.whisper_size)
    else:
        pipeline = OracleIRMWhisperPipeline(weights_manager=weights, whisper_model_size=args.whisper_size)
    ...
    if args.streaming:
        ...
        logger.info("Executing offline separation pipeline...")
    ...
    print("\n" + "=" * 70)
    print(f" PARADIGM 2 EVALUATION REPORT: {output.session_id}")
    print("=" * 70)
